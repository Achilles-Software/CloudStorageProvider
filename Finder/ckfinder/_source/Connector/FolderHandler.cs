...
namespace CKFinder.Connector
{
	public class FolderHandler
	{
		...
		private Achilles.Acme.Storage.IO.DirectoryInfo _FolderInfo;
		private Achilles.Acme.Storage.IO.DirectoryInfo _ThumbsFolderInfo;
		...
		public string ThumbsServerPath
		{
			get
			{
				if ( _ThumbsServerPath == null )
				{
					...
					Achilles.Acme.Storage.IO.Directory.CreateDirectory( _ThumbsServerPath );
				}
				return _ThumbsServerPath;
			}
		}

		public Achilles.Acme.Storage.IO.DirectoryInfo FolderInfo
		{
			get
			{
				if ( _FolderInfo == null )
					_FolderInfo = new Achilles.Acme.Storage.IO.DirectoryInfo( this.ServerPath );
				return _FolderInfo;
			}
		}

		public Achilles.Acme.Storage.IO.DirectoryInfo ThumbsFolderInfo
		{
			get
			{
				if ( _ThumbsFolderInfo == null )
					_ThumbsFolderInfo = new Achilles.Acme.Storage.IO.DirectoryInfo( this.ThumbsServerPath );
