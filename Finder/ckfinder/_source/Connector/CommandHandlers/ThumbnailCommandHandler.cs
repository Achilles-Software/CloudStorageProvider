...
namespace CKFinder.Connector.CommandHandlers
{
	internal class ThumbnailCommandHandler : CommandHandlerBase
	{
		...
		public override void SendResponse( System.Web.HttpResponse response )
		{
			...
			if ( !Achilles.Acme.Storage.IO.File.Exists( thumbFilePath ) )
			{
				string sourceFilePath = System.IO.Path.Combine( this.CurrentFolder.ServerPath, fileName );

				if ( !Achilles.Acme.Storage.IO.File.Exists( sourceFilePath ) )
				{
			...
			Achilles.Acme.Storage.IO.FileInfo thumbfile = new Achilles.Acme.Storage.IO.FileInfo( thumbFilePath ) ;
            ...
            // TJT: We need to access the file stream and read the contents into memory
            System.IO.Stream thumbStream = Achilles.Acme.Storage.IO.File.OpenRead( thumbFilePath );

            int bufferSize = (int)thumbStream.Length;
            if ( bufferSize > 0 )
            {
                byte[] buffer = new byte[bufferSize];
                int count = thumbStream.Read( buffer, 0, bufferSize );
